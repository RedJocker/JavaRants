...
- =main=
  ...
  All java programs need to have a public static main method defined
  on the class that is going to act as Main class,
  that on our example is =class HelloWord=.
  Which class is going to act as Main class is defined on the
  command we used to run the program, =java HelloWorld=
- =(String[] args)=
